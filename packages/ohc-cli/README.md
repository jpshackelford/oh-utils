# ohc-cli - OpenHands Cloud CLI

Administrative command-line tools for managing OpenHands Cloud and Enterprise deployments.

## Overview

`ohc-cli` provides the `ohc` command - a CLI tool for administrators and developers who need to manage OpenHands deployments, conversations, and debug issues. It provides server configuration, conversation management, and debugging capabilities.
...
This will also install the `ohc` library as a dependency.
...
- **ohc**: The underlying Python library for programmatic API access
